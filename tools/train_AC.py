...
def load_model(checkpoint_path, actor_state_size, action_size):
    ...
    checkpoint = torch.load(checkpoint_path, encoding='iso-8859-1')
...
def main(args):
    ...
    for e in count():
        ...
        for t in count():
            ...
            for myi in count():
                candidate_action = actions_cat.sample()
                if int(candidate_action) != int(action):
                    break
        ...
        # [n, 1] all of these 4 variables belowSL_first_train = False
        ...
        if e % opt['losses_log_every'] == 0:
            print('e[%s], loss=%.3f, actor_lr=%.2E, time=%.3f h' % (e, actor_loss.data.item(), actor_lr, total_time))
            data_time, model_time = 0, 0

            f = open("./PG_loss_log", "a")
            f.write(str('e[%s], loss=%.3f, actor_lr=%.2E, time=%.3f h' % (e, actor_loss.data.item(), actor_lr, total_time)) + "\n")
            f.close()
        ...
        if (e % opt['save_every'] == 0) and (e > 0) or iter == opt['max_iters']:
            ...
            if best_val_score is None or current_score > best_val_score:
                ...
                print('critic model saved to %s' % critic_checkpoint_path)
