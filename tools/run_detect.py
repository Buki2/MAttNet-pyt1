...
def cls_to_detections(scores, boxes, imdb, nms_thresh, conf_thresh):
  ...
  for cls_ind, class_name in enumerate(imdb.classes[1:]):
    ...
    inds = np.where(dets[:, -1] >= float(conf_thresh))[0]
    dets = dets[inds, :]
...
def main(args):
  ...
  for image_id, image in refer.Imgs.items():
    ...
    scores, boxes = im_detect(mrcn.net, im)
    ...
    ## my
    my_dict_rpn_boxes[image_id] = boxes
    my_dict_rpn_box_scores[image_id] = scores
  ...
  ## my save
  with open('./rpn_boxes.pkl', 'wb') as f1:
    pickle.dump(my_dict_rpn_boxes, f1)
  with open('./rpn_box_scores.pkl', 'wb') as f1:
    pickle.dump(my_dict_rpn_box_scores, f1)
